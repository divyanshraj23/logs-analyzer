...
def read_xlsx(file_path):
    """Read and extract text from an .xlsx file."""
    try:
        # Load the Excel file
        df = pd.read_excel("./Logs_data.xlsx", sheet_name=None)  # Read all sheets
        # Combine all sheets into a single string
        text = []
        for sheet_name, sheet_data in df.items():
            text.append(f"Sheet: {sheet_name}")
            text.append(sheet_data.to_string(index=False))  # Convert sheet to string
        return "\n".join(text)
    except Exception as e:
        raise FileNotFoundError(f"Error reading the file: {e}")
...
def main():
    # File path to the input file (either .docx or .xlsx)
    file_path = "D:\wazuh_logs\py\Logs_data.xlsx"  # Change this to your file

    # Step 1: Determine the file type and read the data
    if file_path.endswith(".docx"):
        try:
            log_data = read_docx(file_path)
            print("Log data extracted successfully from .docx!")
        except FileNotFoundError as e:
            print(e)
            return
    elif file_path.endswith(".xlsx"):
        try:
            log_data = read_xlsx(file_path)
            print("Log data extracted successfully from .xlsx!")
        except FileNotFoundError as e:
            print(e)
            return
    else:
        print("Unsupported file type. Please provide a .docx or .xlsx file.")
        return
    ...
    output_file = "cybersecurity_output_1.csv"
    try:
